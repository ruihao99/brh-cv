...
\begin{rSection}{代表性论著}
    ...
    \begin{enumerate}[series=myenum]
        ...
        \item \fullcite{Maosh2025} \showpdf{https://arxiv.org/pdf/2412.19097}（共同一作）\label{pap:fortran1}
    ...
    \begin{enumerate}[resume*=myenum]
        ...
        \item \fullcite{Zhuang2022} \showcode{https://github.com/robinzyb/ECToolkits}（共同一作）\label{pap:cp2k1}
    \end{enumerate}
\end{rSection}
...
\begin{tabular}{ @{} >{\bfseries}l @{\hspace{6ex}} p{11cm} }
    编程与软件： & Python（主要），C++（论文~\ref{pap:cpp1},~\ref{pap:cpp2}），Fortran（论文~\ref{pap:fortran1}），MPI（论文~\ref{pap:mpi1},~\ref{pap:mpi2}），CP2K（论文~\ref{pap:cp2k1},~\ref{pap:cp2k2}），Q-Chem（开发者，论文~\ref{pap:q-chem1}） \\
    语言能力： & 中文（母语），英文（熟练，TOEFL iBT: 103，2021年11月）
\end{tabular}
\end{rSection}
